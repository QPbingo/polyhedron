...
export function Workspace({current,project,host,canUse,busy,connection,rpc,terminal,terminalVersion,fontSize,palette,details,searchOpen,terminalQuery,hasProjects,hasHosts,onToggleDetails,onReload,onToggleSearch,onSearchQuery,onFind,onCopy,onSession,onError,onConnectHost,onClaim,onInterrupt,onRename,onHistory,onResume,onArchive,onTerminate}:Props){
 return <section className="workspace-card" id="terminal-workspace" aria-label="当前会话工作区">
  <section className="terminal-frame" aria-label="Agent 终端">
   ...
   {searchOpen&&<form className="terminal-search glass-control" onSubmit={event=>{event.preventDefault();onFind()}}><input aria-label="在终端中查找" value={terminalQuery} onChange={event=>onSearchQuery(event.target.value)} autoFocus placeholder="搜索终端输出"/><button type="button" onClick={()=>onFind(true)}>上一个</button><button>下一个</button><ToolButton label="关闭终端搜索" icon="close" onClick={onToggleSearch}/></form>}
   {current?<TerminalView key={`${current.id}:${current.runtimeEpoch}:${terminalVersion}`} ref={terminal} session={current} rpc={rpc} connection={connection} hostOnline={!!host?.online} fontSize={fontSize} palette={palette} onSession={onSession} onError={onError}/>:<div className="workspace-empty"><Icon name="terminal"/><h1>{hasProjects?'选择一个会话，继续工作。':'从一个项目开始。'}</h1><p>{hasHosts?'将光标移到左侧项目上，即可为该项目新建 Agent 会话。':'在设置中配对你的执行主机。'}</p>{!hasHosts&&<button className="primary" onClick={onConnectHost}>连接执行主机</button>}</div>}
  </section>
