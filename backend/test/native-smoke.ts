...
const config:HostConfig={hostId:randomUUID(),accountId:'smoke',hostToken:randomBytes(32).toString('hex'),masterKey:randomBytes(32).toString('base64url'),ipcToken:randomBytes(32).toString('hex'),relayUrl:'ws://127.0.0.1:3001/ws/host',name:'Native smoke',roots:[root],dataDir:join(dir,'data')};
...
const clientId=randomUUID();async function call(method:string,params:any={}){const grant=await new SignJWT({hostId:config.hostId,clientId,scope:'host',method,sessionId:params.sessionId}).setSubject(config.accountId).setProtectedHeader({alg:'HS256'}).setExpirationTime('30s').sign(new TextEncoder().encode(config.hostToken));return manager.rpc({type:'rpc',requestId:randomUUID(),clientId,grant,method,params:{operationId:randomUUID(),...params}});}
const reports:any[]=[];
try{
 const p=await call('addProject',{name:'Native startup smoke',path:root});
 for(const agent of ['codex','claude']){
  const s=await call('createSession',{projectId:p.id,title:agent+' startup only',agent});await call('attach',{sessionId:s.id,lastAppliedOffset:0});
  await new Promise(r=>setTimeout(r,6000));const snap=await call('attach',{sessionId:s.id,lastAppliedOffset:0});
  reports.push({agent,processState:snap.session.processState,baseOffset:snap.baseOffset,headOffset:snap.headOffset,screenBytes:Buffer.byteLength(snap.data),nativeIdCaptured:!!snap.session.nativeSessionId,activity:snap.session.activity,containsTrustPrompt:/trust|信任|Trust/.test(snap.data),containsConfigurationError:/error (?:loading|parsing)|unknown field|invalid config|unexpected argument/i.test(snap.data)});
  if(snap.session.processState==='running'){const owner=await call('claimControl',{sessionId:s.id,runtimeOffset:s.runtimeOffset});await call('terminate',{sessionId:s.id,runtimeOffset:s.runtimeOffset,controlOffset:owner.controlOffset,confirm:true});await new Promise(r=>setTimeout(r,1800));}
 }
