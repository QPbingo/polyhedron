...
- 详情中可重命名、中断、确认结束进程、查看 Host 本地永久 journal 历史、恢复有精确原生 ID 的历史会话，以及归档。
- 设置中包含六套配色、字号、主机配对/解绑、CLI 检测、运行时防休眠和本地历史磁盘告警阈值。
...
生产 CSP 在 `frontend/nginx.conf` 中实际输出：脚本仅同源，禁止对象嵌入及被其他站点 iframe 嵌入，允许 xterm 动态样式。TLS/WSS 之上还使用浏览器到 Host 的应用层内容加密；正常工作的中继只转发密文，不保存项目、会话或终端投影。浏览器首次连接以 TOFU 固定 Host 指纹，高安全环境应带外核对首次指纹。当前同账号免审批接入使 Relay 仍是授权信任边界：被主动攻陷的 Relay 持有 Host 授权凭据，可以另开自己的加密通道并发起操作；现有加密保证针对持久化泄露和被动窥探，不等价于零信任 Relay。
