...
状态：设计已确认并实现；验证结果见 `docs/implementation/VERIFICATION.md`。
...
同账号新设备可以直接连接这一产品要求意味着：如果主机指纹首次完全通过 Relay 获取，系统属于 trust-on-first-use。它能防止 Relay 数据库泄露和连接后的被动窥探，但不能在首次连接时完全防御主动恶意 Relay 替换身份。并且 Relay 持有用于签发 Host grant 的凭据，主动攻陷的 Relay 可以为自己的临时客户端密钥签发 grant、另开通道并发起操作；浏览器固定真实 Host 指纹并不能阻止这条独立授权路径。因此当前 Relay 是授权信任边界，不是零信任组件。
...
- 在没有引入 Host 注册的账号/设备签名根或本地设备审批前，不得宣称能抵御主动恶意 Relay 发起控制操作；
